...
`NativeAd.createForAdRequest` returns a promise that **rejects** when the underlying ad request fails (for example invalid ad unit ID, network error, or no fill).
On Android this matches the existing iOS behavior: the native module rejects the load promise instead of leaving it pending.

Rejected errors match the `NativeAdLoadError` type. Use `reason` for programmatic handling;
`code` retains the namespaced native code and `message` contains the underlying SDK / platform failure text.
See [NativeAdLoadError](https://github.com/invertase/react-native-google-mobile-ads/blob/main/packages/core/src/types/NativeAdLoadError.ts) for the type definition.
...
NativeAd.createForAdRequest(TestIds.NATIVE)
  ...
  .catch(error => {
    ...
    // error.reason describes the stable failure category (for example 'no-fill')
    // error.message describes the platform failure
  });
```
...
<Warning>
  ...
  component. See more in [Caveats: `NativeAsset` placement](/native-ads#nativeasset-placement)
  section.
</Warning>
